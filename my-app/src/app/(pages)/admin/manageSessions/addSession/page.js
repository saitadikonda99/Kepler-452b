...
const page = () => {
  ...
  const handleUserSearch = async (value, field) => {
    ...
    try {
      const response = await axios.get(`/api/users/search/${sessionData.clubId}?query=${value}`, {
        headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (sessionData.academicYearId && sessionData.clubId) {
        try {
          setIsLoading(true);
          const courseResponse = await axios.get(
            `/api/getCourses/club/${sessionData.clubId}/year/${sessionData.academicYearId}`,
            {
              headers: {
                "Content-Type": "application/json",
              },
